...
# TODO: this needs to be a request against if the component exists in gitlab
...
makedirs(hash_dir)
# set mtime if provided by the user
if "mtime" in form:
    mtime_str = form.getvalue("mtime")
    try:
        mtime = int(mtime_str)
    except ValueError:
        send_error("Invalid value sent for mtime %r" % mtime_str)
